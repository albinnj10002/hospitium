...
$date=date("Y/m/d");
                //echo $date;
           $sql="UPDATE appoinment set ap_status=1 where ap_date<='$date'";
           //var_dump($sql);
           if(mysqli_query($con,$sql))
           {

$s="SELECT * FROM appoinment WHERE ap_hospital='$hos' AND ap_status=0";
//var_dump($s);
      if(!$stmt=mysqli_query($con,$s))
      {
          ...
          <div class="feature-body service3"><a href="#url">
            ...
            <div class="feature-info mt-4"><a href="#url">
               ...
               <p class="feature-text">Book Date: <?php echo $date;?></p>
                ...
                <?php
                   $s1="SELECT * FROM user WHERE ul_id=$apl_id";
//var_dump($s);
    if(!$stmt=mysqli_query($con,$s1))
    {
      die("prepare statement error1");
    }
    $result=mysqli_fetch_array($stmt);
    $gender=$result['u_gender'];  ?> 
          ...
          <a href="bookappo.php?ap_id=<?php echo $ap_id;?>" class="btn btn-primary">Book</a>
          
          <a href="deleteappoin.php?ap_id=<?php echo $ap_id;?>" class="btn btn-primary">Delete</a>
        </div>
            </div>
          </div>
